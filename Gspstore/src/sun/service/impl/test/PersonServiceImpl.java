...
import sun.service.test.PersonServiceI;
...
public class PersonServiceImpl extends BaseServiceImpl implements PersonServiceI {
	...
	private BaseDaoI<PersonEntity> personEntityDao;

	public BaseDaoI<PersonEntity> getPersonEntityDao() {
		return personEntityDao;
	}
	@Autowired
	public void setPersonEntityDao(BaseDaoI<PersonEntity> personEntityDao) {
	...
	private String addWhere(PersonPage personPage, String hql, List<Object> values) {
		...
		SearchSqlGenerateUtil.createSearchParamsHql(hqlbf, values, personEntity);
		hql = hqlbf.toString();
		//-----------------------------------------------------
	...
	public void update(PersonPage personPage) throws Exception {
	    ...
	    if(t != null) {
			MyBeanUtils.copyBeanNotNull2Bean(personPage, t);
		}
	}
