...
<script type="text/javascript" charset="utf-8">
	...
	$(function() {
		...
		treegrid = $('#treegrid').treegrid({
			url : '${entityName?uncap_first}Action!treegrid.action',
			toolbar : [ {
				text : '展开',
				iconCls : 'icon-redo',
				handler : function() {
					...
					if (node) {
						treegrid.treegrid('expandAll', node.${jeecg_table_id});
					} else {
				...
				handler : function() {
					var node = treegrid.treegrid('getSelected');
					if (node) {
						treegrid.treegrid('collapseAll', node.${jeecg_table_id});
					} else {
				...
				handler : function() {
					if (editRow != undefined) {
						treegrid.treegrid('endEdit', editRow.${jeecg_table_id});
					}
				}
			}, '-', {
				text : '取消编辑',
				...
				handler : function() {
					if (editRow) {
						treegrid.treegrid('cancelEdit', editRow.${jeecg_table_id});
						var p = treegrid.treegrid('getParent', editRow.${jeecg_table_id});
						if (p) {
							treegrid.treegrid('reload', p.${jeecg_table_id});
						} else {
							treegrid.treegrid('reload');
						}
						editRow = undefined;
			...
			treeField : 'cname',
			frozenColumns : [ [ {
				title : '${jeecg_table_id}',
				field : '${jeecg_table_id}',
				width : 150,
				hidden : true
			}, {
				field : 'cname',
			...
			},
			{
				...
				editor : {
					type : 'combotree',
					options : {
						url : '${entityName?uncap_first}Action!${entityName?uncap_first}TreeRecursive.action',
						animate : false,
			...
			onDblClickRow : function(row) {
				...
				if (editRow == undefined) {
					treegrid.treegrid('beginEdit', row.${jeecg_table_id});
					editRow = row;
					editType = 'edit';
					treegrid.treegrid('unselectAll');
				}
				...
				$.ajax({
					...
					success : function(r) {
						if (r.success) {
							...
							treegrid.treegrid('beginEdit', editRow.${jeecg_table_id});
			...
			onContextMenu : function(e, row) {
				...
				$(this).treegrid('select', row.${jeecg_table_id});
				$('#menu').menu('show', {
					left : e.pageX,
					top : e.pageY
				});
	...
	function edit() {
		var node = treegrid.treegrid('getSelected');
		if (node && node.${jeecg_table_id}) {
			if (editRow != undefined) {
				treegrid.treegrid('endEdit', editRow.${jeecg_table_id});
			}

			if (editRow == undefined) {
				treegrid.treegrid('beginEdit', node.${jeecg_table_id});
				editRow = node;
				editType = 'edit';
			}
		} else {
	...
	function append() {
		...
		if (editRow == undefined) {
			var node = treegrid.treegrid('getSelected');
			var row = [ {
				${jeecg_table_id} : sy.UUID(),
				cname : '${ftl_description}',
				curl : '',
				cseq : 10,
				cpid : node == null ? '' : node.${jeecg_table_id}
			} ];
			treegrid.treegrid('append', {
				parent : node == null ? '' : node.${jeecg_table_id},
				data : row
			});

			editRow = row[0];
			editType = 'add';
			treegrid.treegrid('select', editRow.${jeecg_table_id});
			treegrid.treegrid('beginEdit', editRow.${jeecg_table_id});
		}
	}
	function remove() {
			...
			$.messager.confirm('询问', '您确定要删除【' + node.cname + '】？', function(b) {
				if (b) {
					$.ajax({
						...
						data : {
							${jeecg_table_id} : node.${jeecg_table_id}
						},
						cache : false,
						dataType : "json",
						success : function(r) {
